...
class TesolloDeltoRlEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: TesolloDeltoRlEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        if getattr(self.cfg, "use_manual_joint_cfg", False):
            manual_pos = (
                torch.tensor(self.cfg.hand_position, dtype=torch.float, device=self.device)
                .view(1, -1)
                .repeat(self.num_envs, 1)
            )
            manual_upper = (
                torch.deg2rad(torch.tensor(self.cfg.hand_upper_limits, dtype=torch.float, device=self.device))
                .view(1, -1)
                .repeat(self.num_envs, 1)
            )
            manual_lower = (
        ...
        self.object_local_pos = quat_apply(
            quat_conjugate(self.hand_base_rot), object_default_pos_env - self.hand_base_pos
        )
        ...
        # 手内奖励目标位置（手部根部坐标系）
        in_hand_local_offset = torch.tensor(
            self.cfg.in_hand_local_offset, dtype=torch.float, device=self.device
        ).repeat((self.num_envs, 1))
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        self.target_pos[:, self.actuated_dof_indices] = (
            self.target_pos[:, self.actuated_dof_indices] + self.action_scale * self.raw_actions
        )
        # 限制在关节上下限内
    ...
    def _reset_target_pose(self, env_ids):
        ...
        self.goal_pos[env_ids] = self.hand_base_pos[env_ids] + quat_apply(
            self.hand_base_rot[env_ids],
            self.object_local_pos[env_ids],
        )
